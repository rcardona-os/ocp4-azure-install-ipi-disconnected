...
- Creates a credentials configuration file called $`\textcolor{LightSalmon} {\text{osServicePrincipal.json}}`$, and store it file in the  $`\textcolor{LightSalmon} {\text{ ~/.azure/}}`$ directory. This ensures that the installation program can load the profile when it is creating an OpenShift Container Platform cluster on the target platform.
