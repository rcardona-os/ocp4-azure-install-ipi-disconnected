...
- Creates a credentials configuration file called $`\textcolor{darkblue}   {\text{30}}  {\text{ osServicePrincipal.json }}`$, and store it file in the `${HOME}/.azure/` directory. This ensures that the installation program can load the profile when it is creating an OpenShift Container Platform cluster on the target platform.
